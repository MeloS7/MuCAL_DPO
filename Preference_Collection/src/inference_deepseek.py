...
def inference_llm(dataset, batch_size=16, model_name="Qwen/Qwen2.5-7B-Instruct", cache_dir=None):
    ...
    # For deepseek-r1-distill-qwen-7B, the pad token is eos token
...
"""
    ...
    for i in tqdm(range(0, len(dataset), batch_size)):
        ...
        generated_ids_batch = model.generate(
            **model_inputs_batch,
            max_new_tokens=1024,
            temperature=0.6, # For deepseek-r1-distill-qwen-7B, the temperature is 0.6
            do_sample=True,
            top_p=0.95,
        )
        ...
        # 修改这里：只保留</think>\n\n后面的内容
        response_batch = [response.split("</think>\n\n")[-1] if "</think>\n\n" in response else response for response in response_batch]
