...
if __name__ == "__main__":
    ...
    args.add_argument('-hn', '--use_hard_negatives', action='store_true', help='Flag to use hard negatives')
    ...
    args.add_argument('-hn_num', '--num_hard_negatives', type=int, default=6, help='Number of hard negatives')
    args.add_argument('-bd', '--bidirection', action='store_true', help='Flag to use bidirectional training')
    args.add_argument('-langs', '--languages', type=str, default='en', help='Languages to use')
    ...
    use_bidirection = args.bidirection
    
    if args.use_wandb:
        # Initialize W&B
        wandb.init(
            project="Multi-Align-G2T",
            name=f"BiEncoder_lr_{learning_rate}_batch_{train_batch_size}_epoch_{epochs}_cosine_p2_hard_neg_{num_hard_negatives}_langs_{args.languages}",
            group="BiEncoder",
            config={
    ...
    # model = 'BAAI/bge-m3'
    if load_path:
        encoder = BiEncoder.load(load_path)
    else:
        encoder = BiEncoder(model)
    ...
    # Train the model
    if train:
        trainer.train(data_loader, use_hard_negatives, num_hard_negatives)
        # Save the checkpoint
    ...
    if evaluate:
      ...
      # =========== T2G Part ==============
        ...
        mono_lang_results, mono_mrr = ranker.compute_graph_to_monolang_text_rank_n(n=[1, 3, 10])
        
        # Print Monolingual Rank@N results and average ranks for each language
        print("Monolingual G2T - Rank@N Accuracy Results by Language:")
        for lang, results in mono_lang_results.items():
            print(f"\nLanguage: {lang}")
            for rank in [1, 3, 10]: 
